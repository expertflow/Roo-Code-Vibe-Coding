...
<#
.SYNOPSIS
  Run docs/sql/drop-legacy-transaction-expense-field.sql as break-glass bs4_dev.
...
.NOTES
  ...
  Runs psql from PATH, or — if psql is missing — `docker run postgres:16-alpine`.
#>
param(
...
$sql = Join-Path $root 'docs\sql\drop-legacy-transaction-expense-field.sql'
...
Options:
  ...
  • Run the SQL manually in DBeaver / Cloud SQL Studio as bs4_dev:
      docs/sql/drop-legacy-transaction-expense-field.sql
'@
}
