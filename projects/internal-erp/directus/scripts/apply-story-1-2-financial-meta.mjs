#!/usr/bin/env node
/**
 * Story 1.2 — Apply collection labels, display templates, relations, and field interfaces
 * to the nine financial collections via Directus REST API.
 *
 * Usage:
 *   DIRECTUS_URL=http://localhost:8055 DIRECTUS_ADMIN_EMAIL=... DIRECTUS_ADMIN_PASSWORD=... node apply-story-1-2-financial-meta.mjs
 *   # or STATIC_TOKEN=... (admin static token)
 *   node apply-story-1-2-financial-meta.mjs --dry-run
 *
 * Prerequisites: Directus connected to bidstruct4; tables introspected as collections.
 */
...
async function api(base, path, opts = {}) {
  ...
  if (dryRun) {
    console.log(`[dry-run] ${opts.method || 'GET'} ${url}`, opts.body ? JSON.stringify(opts.body).slice(0, 200) : '');
    return { data: null };
  }
...
/** Keys safe to keep when stripping file/image meta (avoid re-merging broken validation → `.match` 500s). */
const PLAIN_FIELD_META_PASSTHROUGH = [
  'note',
  'hidden',
  'readonly',
  'required',
...
/**
 * Directus often **merges** field PATCH meta and ignores `null` for display — spreading `current.meta`
 * keeps file `display` / `validation`. Rebuild a minimal meta blob for varchar fields mistaken for files.
 */
...
async function main() {
  ...
  if (!dryRun) {
    console.log('Story 1.2 — applying financial collection metadata to', base);
  }
  ...
  for (const rel of FINANCIAL_RELATIONS) {
    const key = relationKey(rel);
    if (existingRelKeys.has(key)) continue;
    // Directus REST expects these keys (not many_collection / many_field)
    const body = {
    ...
    if (dryRun) {
      console.log('[dry-run] POST /relations', body);
      continue;
    }
    try {
      ...
      console.warn('Relation skip/fail (may already exist):', key, e.message);
    }
  }
  ...
  for (const col of FINANCIAL_COLLECTIONS) {
    ...
    const fieldsRes = dryRun ? { data: columns.map((c) => ({ field: c.name, type: c.type })) } : await api(base, `/fields/${encodeURIComponent(col)}`, { headers: auth });
    ...
    for (const row of columns) {
      ...
      if (!dryRun) {
        ...
        let meta;
        if (overrides.clearSpecial) {
          meta = metaForPlainTextAfterClearingFile(current.meta, iface, options, translation);
        } else {
          meta = {
            ...(current.meta || {}),
            interface: iface,
        ...
        if (iface === 'm2o' || String(iface).includes('m2o')) {
          meta.special = Array.from(new Set([...(meta.special || current.meta?.special || []), 'm2o']));
        }
  ...
  console.log(dryRun ? 'Dry run complete.' : 'Story 1.2 apply complete. Verify in Directus Admin (AC 1–4).');
}
