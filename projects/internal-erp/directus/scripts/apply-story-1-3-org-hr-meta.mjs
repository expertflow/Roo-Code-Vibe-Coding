#!/usr/bin/env node
/**
 * Story 1.3 — Apply collection labels, display templates, relations, and field interfaces
 * for 11 org/HR tables via Directus REST API.
 *
 * Usage:
 *   DIRECTUS_URL=http://localhost:8055 DIRECTUS_ADMIN_EMAIL=... DIRECTUS_ADMIN_PASSWORD=... node apply-story-1-3-org-hr-meta.mjs
 *   node apply-story-1-3-org-hr-meta.mjs --dry-run
 */
...
async function main() {
  ...
  if (!dryRun) {
    console.log('Story 1.3 — applying org/HR collection metadata to', base);
  }
  ...
  for (const rel of ORG_HR_RELATIONS) {
  ...
  console.log(
    dryRun
      ? 'Dry run complete.'
      : 'Story 1.3 apply complete. Verify in Directus Admin (AC 1–4).',
  );
}
